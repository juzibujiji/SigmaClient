...
public class NoAddonHolder extends MainMenuHolder {
   ...
   protected void init() {
      ...
      this.addButton(new Button(this.width / 2 - 100, j + 72 + 12 + 24, 200, 20,
            new StringTextComponent("Switch"), (btn) -> {
               Client.getInstance().guiManager.handleScreen(new SwitchScreen());
            }));
   }
   ...
   public void render(MatrixStack matrices, int var2, int var3, float delta) {
      ...
      drawString(matrices, this.font, "No Addons - SIGMA", 87, this.height - 10,
            RenderUtil2.applyAlpha(ClientColors.LIGHT_GREYISH_BLUE.getColor(), 0.4F));
      GL11.glPushMatrix();
      GL11.glTranslatef(88.0F, (float) (this.height - 10), 0.0F);
      GL11.glScalef(0.5F, 0.5F, 1.0F);
      GL11.glTranslatef(-88.0F, (float) (-(this.height - 10)), 0.0F);
      drawString(matrices, this.font, "PROD", 280, this.height - 10,
            RenderUtil2.applyAlpha(ClientColors.LIGHT_GREYISH_BLUE.getColor(), 0.4F));
      GL11.glPopMatrix();
   }
