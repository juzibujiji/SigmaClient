...
public class ChunkPrimer implements IChunk {
    ...
    private final ShortList[] packedPositions = new ShortList[WorldHeightHelper.getSectionCount()];
    ...
    public ChunkPrimer(ChunkPos pos, UpgradeData upgradeData, @Nullable ChunkSection[] sections,
            ChunkPrimerTickList<Block> pendingBlockTicks, ChunkPrimerTickList<Fluid> pendingFluidTicks) {
        this.pos = pos;
        ...
        if (sections != null) {
            ...
            } else if (sections.length == 16) {
                // Handle legacy 16-section input by copying to offset 4 (starting at Y=0)
                System.arraycopy(sections, 0, this.sections, WorldHeightHelper.getSectionOffset(), 16);
            } else {
                LOGGER.warn("Could not set level chunk sections, array length is {} instead of {}", sections.length,
    ...
    public BlockState getBlockState(BlockPos pos) {
        ...
        if (World.isYOutOfBounds(i)) {
            return Blocks.VOID_AIR.getDefaultState();
                        ...
                        int sectionIdx = (i >> 4) + WorldHeightHelper.getSectionOffset();
            if (sectionIdx < 0 || sectionIdx >= this.sections.length) return Blocks.AIR.getDefaultState();
            ChunkSection chunksection = this.getSections()[sectionIdx];
            return ChunkSection.isEmpty(chunksection) ? Blocks.AIR.getDefaultState()
    ...
    public FluidState getFluidState(BlockPos pos) {
        ...
        if (World.isYOutOfBounds(i)) {
            ...
            int sectionIdx = (i >> 4) + WorldHeightHelper.getSectionOffset();
            if (sectionIdx < 0 || sectionIdx >= this.sections.length) return Fluids.EMPTY.getDefaultState();
            ChunkSection chunksection = this.getSections()[sectionIdx];
            return ChunkSection.isEmpty(chunksection) ? Fluids.EMPTY.getDefaultState()
    ...
    public ShortList[] getPackedLightPositions() {
        ShortList[] ashortlist = new ShortList[WorldHeightHelper.getSectionCount()];

        for (BlockPos blockpos : this.lightPositions) {
            IChunk.getList(ashortlist, (blockpos.getY() >> 4) + WorldHeightHelper.getSectionOffset()).add(packToLocal(blockpos));
        }
    ...
    public BlockState setBlockState(BlockPos pos, BlockState state, boolean isMoving) {
        ...
        int sectionIdx = (j >> 4) + WorldHeightHelper.getSectionOffset();
        if (!WorldHeightHelper.isYOutOfBounds(j) && sectionIdx >= 0 && sectionIdx < this.sections.length)
        {
            if (this.sections[sectionIdx] == Chunk.EMPTY_SECTION && state.isIn(Blocks.AIR))
            {
                return state;
            } else {
                if (state.getLightValue() > 0) {
                    this.lightPositions.add(new BlockPos((i & 15) + this.getPos().getXStart(), j,
                            (k & 15) + this.getPos().getZStart()));
                }

                ChunkSection chunksection = this.getSection(sectionIdx);
                BlockState blockstate = chunksection.setBlockState(i & 15, j & 15, k & 15, state);

                if (this.status.isAtLeast(ChunkStatus.FEATURES) && state != blockstate
                        && (state.getOpacity(this, pos) != blockstate.getOpacity(this, pos)
                                || state.getLightValue() != blockstate.getLightValue() || state.isTransparent()
    ...
    public ChunkSection getSection(int sectionId) {
        if (this.sections[sectionId] == Chunk.EMPTY_SECTION) {
            this.sections[sectionId] = new ChunkSection((sectionId - WorldHeightHelper.getSectionOffset()) << 4);
        }
    ...
    public void markBlockForPostprocessing(BlockPos pos) {
        if (!World.isOutsideBuildHeight(pos)) {
            IChunk.getList(this.packedPositions, (pos.getY() >> 4) + WorldHeightHelper.getSectionOffset()).add(packToLocal(pos));
        }
    }
