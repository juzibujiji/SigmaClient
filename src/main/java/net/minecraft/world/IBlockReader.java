...
public interface IBlockReader {
    ...
    default int getHeight() {
        return com.mentalfrostbyte.jello.util.game.world.WorldHeightHelper.getMaxY();
    }
    ...
    default BlockRayTraceResult rayTraceBlocks(RayTraceContext context) {
        return doRayTrace(context, (p_217297_1_, p_217297_2_) -> {
            ...
            BlockRayTraceResult blockraytraceresult = this.rayTraceBlocks(vector3d, vector3d1, p_217297_2_, voxelshape,
                    blockstate);
            ...
            double d0 = blockraytraceresult == null ? Double.MAX_VALUE
                    : p_217297_1_.getStartVec().squareDistanceTo(blockraytraceresult.getHitVec());
            double d1 = blockraytraceresult1 == null ? Double.MAX_VALUE
            ...
            return BlockRayTraceResult.createMiss(p_217302_0_.getEndVec(),
                    Direction.getFacingFromVector(vector3d.x, vector3d.y, vector3d.z),
    ...
    default BlockRayTraceResult rayTraceBlocks(Vector3d startVec, Vector3d endVec, BlockPos pos, VoxelShape shape,
            BlockState state) {
        BlockRayTraceResult blockraytraceresult = shape.rayTrace(startVec, endVec, pos);

        if (blockraytraceresult != null) {
            BlockRayTraceResult blockraytraceresult1 = state.getRayTraceShape(this, pos).rayTrace(startVec, endVec,
                    pos);

            if (blockraytraceresult1 != null && blockraytraceresult1.getHitVec().subtract(startVec)
                    .lengthSquared() < blockraytraceresult.getHitVec().subtract(startVec).lengthSquared()) {
    ...
    static <T> T doRayTrace(RayTraceContext context, BiFunction<RayTraceContext, BlockPos, T> rayTracer,
            Function<RayTraceContext, T> missFactory) {
