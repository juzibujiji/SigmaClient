...
public class DisconnectedScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addButton(
                new Button(this.width / 2 - 100, Math.min(this.height / 2 + this.textHeight / 2 + 9, this.height - 30),
                        200, 20, new TranslationTextComponent("gui.toMenu"), (p_213033_1_) -> {
                            this.minecraft.displayGuiScreen(this.nextScreen);
                        }));

        if (Client.getInstance().moduleManager.getModuleByClass(AutoReconnect.class)
                .getBooleanValueFromSettingName("Reconnect button")) {
            this.addButton(new Button(this.width / 2 - 100,
                    Math.min(this.height / 2 + this.textHeight / 2 + 9, this.height - 30) + 25, 200, 20,
                    new TranslationTextComponent("Reconnect"), (p_213033_1_) -> {
                        ServerData serverData = AutoReconnect.serverData;
                        if (serverData != null)
                            this.minecraft
                                    .displayGuiScreen(new ConnectingScreen(this, Minecraft.getInstance(), serverData));
                    }));
        }
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredString(matrices, this.font, this.title, this.width / 2,
