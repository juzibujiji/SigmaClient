...
public class ChunkRenderDispatcher {
    ...
    private final PriorityQueue<ChunkRenderDispatcher.ChunkRender.ChunkRenderTask> renderTasks = Queues
            .newPriorityQueue();
    ...
    private static final boolean FORGE_CAN_RENDER_IN_LAYER_BS = Reflector.ForgeRenderTypeLookup_canRenderInLayerBs
            .exists();
    private static final boolean FORGE_CAN_RENDER_IN_LAYER_FS = Reflector.ForgeRenderTypeLookup_canRenderInLayerBs
            .exists();
    private static final boolean FORGE_SET_RENDER_LAYER = Reflector.ForgeHooksClient_setRenderLayer.exists();
    public static int renderChunksUpdated;

    public ChunkRenderDispatcher(World worldIn, WorldRenderer worldRendererIn, Executor executorIn, boolean java64bit,
            RegionRenderCacheBuilder fixedBuilderIn) {
        this(worldIn, worldRendererIn, executorIn, java64bit, fixedBuilderIn, -1);
    }

    public ChunkRenderDispatcher(World p_i242107_1_, WorldRenderer p_i242107_2_, Executor p_i242107_3_,
            boolean p_i242107_4_, RegionRenderCacheBuilder p_i242107_5_, int p_i242107_6_) {
        this.world = p_i242107_1_;
        this.worldRenderer = p_i242107_2_;
        int i = Math.max(1,
                (int) ((double) Runtime.getRuntime().maxMemory() * 0.3D)
                        / (RenderType.getBlockRenderTypes().stream().mapToInt(RenderType::getBufferSize).sum() * 4)
                        - 1);
    ...
    private void runTask() {
        if (!this.freeBuilders.isEmpty()) {
            ChunkRenderDispatcher.ChunkRender.ChunkRenderTask chunkrenderdispatcher$chunkrender$chunkrendertask = this.renderTasks
                    .poll();
                ...
                CompletableFuture.runAsync(() -> {
                    ...
                    if (p_lambda$runTask$3_3_ != null) {
                        ...
                        Minecraft.getInstance()
                                .crashed(Minecraft.getInstance().addGraphicsAndWorldToCrashReport(crashreport));
                    } else {
    ...
    public String getDebugInfo() {
        return String.format("pC: %03d, pU: %02d, aB: %02d", this.countRenderTasks, this.uploadTasks.size(),
                this.countFreeBuilders);
    }
    ...
    public boolean updateTransparencyLater(ChunkRenderDispatcher.ChunkRender p_updateTransparencyLater_1_) {
        return this.freeBuilders.isEmpty() ? false
                : p_updateTransparencyLater_1_.resortTransparency(RenderTypes.TRANSLUCENT, this);
    }

    public class ChunkRender implements IForgeRenderChunk {
        public final AtomicReference<ChunkRenderDispatcher.CompiledChunk> compiledChunk = new AtomicReference<>(
        ...
        private final Map<RenderType, VertexBuffer> vertexBuffers = RenderType.getBlockRenderTypes().stream()
                .collect(Collectors.toMap((p_lambda$new$0_0_) -> {
        ...
        private WorldRenderer.LocalRenderInformationContainer renderInfo = new WorldRenderer.LocalRenderInformationContainer(
                this, (Direction) null, 0);
        public AabbFrame boundingBoxParent;

        private boolean isChunkLoaded(BlockPos blockPosIn) {
            return ChunkRenderDispatcher.this.world.getChunk(blockPosIn.getX() >> 4, blockPosIn.getZ() >> 4,
        ...
        public void setPosition(int x, int y, int z) {
            if (x != this.position.getX() || y != this.position.getY() || z != this.position.getZ()) {
                ...
                this.boundingBox = new AxisAlignedBB((double) x, (double) y, (double) z, (double) (x + 16),
                        (double) (y + 16), (double) (z + 16));
        ...
        public boolean resortTransparency(RenderType renderTypeIn, ChunkRenderDispatcher renderDispatcherIn) {
            ...
            if (!chunkrenderdispatcher$compiledchunk.layersStarted.contains(renderTypeIn)) {
                ...
                if (ChunkRenderDispatcher.FORGE) {
                    this.lastResortTransparencyTask = new ChunkRenderDispatcher.ChunkRender.SortTransparencyTask(
                            new ChunkPos(this.getPosition()), this.getDistanceSq(),
                            chunkrenderdispatcher$compiledchunk);
                } else {
                    this.lastResortTransparencyTask = new ChunkRenderDispatcher.ChunkRender.SortTransparencyTask(
                            this.getDistanceSq(), chunkrenderdispatcher$compiledchunk);
                }
        ...
        public ChunkRenderDispatcher.ChunkRender.ChunkRenderTask makeCompileTaskChunk() {
            ...
            if (ChunkRenderDispatcher.FORGE) {
                this.lastRebuildTask = new ChunkRenderDispatcher.ChunkRender.RebuildTask(
                        new ChunkPos(this.getPosition()), this.getDistanceSq(), chunkrendercache);
            } else {
                this.lastRebuildTask = new ChunkRenderDispatcher.ChunkRender.RebuildTask(this.getDistanceSq(),
                        chunkrendercache);
            }
        ...
        public void rebuildChunkLater(ChunkRenderDispatcher dispatcherIn) {
            ChunkRenderDispatcher.ChunkRender.ChunkRenderTask chunkrenderdispatcher$chunkrender$chunkrendertask = this
        ...
        private RenderType[] getFluidRenderLayers(FluidState p_getFluidRenderLayers_1_,
                RenderType[] p_getFluidRenderLayers_2_) {
        ...
        private RenderType[] getBlockRenderLayers(BlockState p_getBlockRenderLayers_1_,
                RenderType[] p_getBlockRenderLayers_2_) {
        ...
        private RenderType fixBlockLayer(IBlockReader p_fixBlockLayer_1_, BlockState p_fixBlockLayer_2_,
                BlockPos p_fixBlockLayer_3_, RenderType p_fixBlockLayer_4_) {
            if (CustomBlockLayers.isActive()) {
                RenderType rendertype = CustomBlockLayers.getRenderLayer(p_fixBlockLayer_1_, p_fixBlockLayer_2_,
                        p_fixBlockLayer_3_);
        ...
        private void postRenderOverlays(RegionRenderCacheBuilder p_postRenderOverlays_1_,
                ChunkRenderDispatcher.CompiledChunk p_postRenderOverlays_2_) {
        ...
        private void postRenderOverlay(RenderType p_postRenderOverlay_1_,
                RegionRenderCacheBuilder p_postRenderOverlay_2_,
                ChunkRenderDispatcher.CompiledChunk p_postRenderOverlay_3_) {
        ...
        private ChunkCacheOF makeChunkCacheOF(BlockPos p_makeChunkCacheOF_1_) {
            ...
            ChunkRenderCache chunkrendercache = this.createRegionRenderCache(ChunkRenderDispatcher.this.world, blockpos,
                    blockpos1, 1);
            return new ChunkCacheOF(chunkrendercache, blockpos, blockpos1, 1);
        }

        public ChunkRenderCache createRegionRenderCache(World p_createRegionRenderCache_1_,
                BlockPos p_createRegionRenderCache_2_, BlockPos p_createRegionRenderCache_3_,
                int p_createRegionRenderCache_4_) {
            return ChunkRenderCache.generateCache(p_createRegionRenderCache_1_, p_createRegionRenderCache_2_,
                    p_createRegionRenderCache_3_, p_createRegionRenderCache_4_, false);
        }

        public ChunkRenderDispatcher.ChunkRender getRenderChunkOffset16(ViewFrustum p_getRenderChunkOffset16_1_,
        ...
        public void setRenderChunkNeighbour(Direction p_setRenderChunkNeighbour_1_,
                ChunkRenderDispatcher.ChunkRender p_setRenderChunkNeighbour_2_) {
        ...
        public AabbFrame getBoundingBoxParent() {
            if (this.boundingBoxParent == null) {
                ...
                if (i1 != i || j1 != j || k1 != k) {
                    AabbFrame aabbframe = ChunkRenderDispatcher.this.worldRenderer
                            .getRenderChunk(new BlockPos(i1, j1, k1)).getBoundingBoxParent();
        ...
        abstract class ChunkRenderTask implements Comparable<ChunkRenderDispatcher.ChunkRender.ChunkRenderTask> {
            ...
            public abstract CompletableFuture<ChunkRenderDispatcher.ChunkTaskResult> execute(
        ...
        class RebuildTask extends ChunkRenderDispatcher.ChunkRender.ChunkRenderTask {
            ...
            public RebuildTask(ChunkPos p_i242106_2_, double p_i242106_3_, ChunkRenderCache p_i242106_5_) {
                super(p_i242106_2_, p_i242106_3_);
                this.chunkRenderCache = p_i242106_5_;
            }

            public CompletableFuture<ChunkRenderDispatcher.ChunkTaskResult> execute(
                    RegionRenderCacheBuilder builderIn) {
                if (this.finished.get()) {
                    ...
                    if (this.finished.get()) {
                        ...
                        chunkrenderdispatcher$compiledchunk.layersStarted.forEach((p_lambda$execute$0_3_) -> {
                            list.add(ChunkRenderDispatcher.this.uploadChunkLayer(
                                    builderIn.getBuilder(p_lambda$execute$0_3_),
                                    ChunkRender.this.getVertexBuffer(p_lambda$execute$0_3_)));
                        });
                        return Util.gather(list).handle((p_lambda$execute$1_2_, p_lambda$execute$1_3_) -> {
                            if (p_lambda$execute$1_3_ != null
                                    && !(p_lambda$execute$1_3_ instanceof CancellationException)
                                    && !(p_lambda$execute$1_3_ instanceof InterruptedException)) {
                                Minecraft.getInstance()
                                        .crashed(CrashReport.makeCrashReport(p_lambda$execute$1_3_, "Rendering chunk"));
                            }
            ...
            private Set<TileEntity> compile(float xIn, float yIn, float zIn,
                ...
                if (!ChunkRender.this.isChunkRegionEmpty(blockpos)) {
                    ...
                    BlockRendererDispatcher blockrendererdispatcher = Minecraft.getInstance()
                            .getBlockRendererDispatcher();

                    for (BlockPosM blockposm : (Iterable<BlockPosM>) BlockPosM.getAllInBoxMutable(blockpos,
                            blockpos1)) {
                        ...
                        if (!blockstate.isAir()) {
                            ...
                            if (ReflectorForge.blockHasTileEntity(blockstate)) {
                                TileEntity tileentity = chunkcacheof.getTileEntity(blockposm,
                                        Chunk.CreateEntityType.CHECK);
                            ...
                            if (!fluidstate.isEmpty()) {
                                RenderType[] arendertype1 = ChunkRender.this.getFluidRenderLayers(fluidstate,
                                        arendertype);
                                ...
                                for (int j = 0; j < arendertype1.length; ++j) {
                                    ...
                                    if (!ChunkRenderDispatcher.FORGE_CAN_RENDER_IN_LAYER_FS
                                            || Reflector.callBoolean(Reflector.ForgeRenderTypeLookup_canRenderInLayerFs,
                                                    fluidstate, rendertype)) {
                                        ...
                                        if (blockrendererdispatcher.renderFluid(blockposm, chunkcacheof, bufferbuilder,
                                                fluidstate)) {
                            ...
                            if (blockstate.getRenderType() != BlockRenderType.INVISIBLE) {
                                RenderType[] arendertype2 = ChunkRender.this.getBlockRenderLayers(blockstate,
                                        arendertype);
                                ...
                                for (int k = 0; k < arendertype2.length; ++k) {
                                    ...
                                    if (!ChunkRenderDispatcher.FORGE_CAN_RENDER_IN_LAYER_BS
                                            || Reflector.callBoolean(Reflector.ForgeRenderTypeLookup_canRenderInLayerBs,
                                                    blockstate, rendertype3)) {
                                        if (ChunkRenderDispatcher.FORGE_SET_RENDER_LAYER) {
                                            Reflector.callVoid(Reflector.ForgeHooksClient_setRenderLayer, rendertype3);
                                        }

                                        rendertype3 = ChunkRender.this.fixBlockLayer(chunkcacheof, blockstate,
                                                blockposm, rendertype3);
                                        ...
                                        matrixstack.translate(
                                                (double) ChunkRender.this.regionDX + (double) (blockposm.getX() & 15),
                                                (double) ChunkRender.this.regionDY + (double) (blockposm.getY() & 15),
                                                (double) ChunkRender.this.regionDZ + (double) (blockposm.getZ() & 15));

                                        if (blockrendererdispatcher.renderModel(blockstate, blockposm, chunkcacheof,
                                                matrixstack, bufferbuilder3, true, random, imodeldata)) {
                                            compiledChunkIn.empty = false;
                    ...
                    if (compiledChunkIn.layersUsed.contains(RenderType.getTranslucent())) {
                        ...
                        bufferbuilder1.sortVertexData((float) ChunkRender.this.regionDX + xIn - (float) blockpos.getX(),
                                (float) ChunkRender.this.regionDY + yIn - (float) blockpos.getY(),
                                (float) ChunkRender.this.regionDZ + zIn - (float) blockpos.getZ());
                        compiledChunkIn.state = bufferbuilder1.getVertexState();
                    }

                    compiledChunkIn.layersStarted.stream().map(builderIn::getBuilder)
                            .forEach(BufferBuilder::finishDrawing);
                    ...
                    for (RenderType rendertype1 : compiledChunkIn.layersStarted) {
                        ...
                        if (bufferbuilder2.animatedSprites != null && !bufferbuilder2.animatedSprites.isEmpty()) {
                            compiledChunkIn.setAnimatedSprites(rendertype1,
                                    (BitSet) bufferbuilder2.animatedSprites.clone());
                        }
                    }
            ...
            private <E extends TileEntity> void handleTileEntity(ChunkRenderDispatcher.CompiledChunk compiledChunkIn,
                    Set<TileEntity> tileEntitiesIn, E tileEntityIn) {
                TileEntityRenderer<E> tileentityrenderer = TileEntityRendererDispatcher.instance
                        .getRenderer(tileEntityIn);
        ...
        class SortTransparencyTask extends ChunkRenderDispatcher.ChunkRender.ChunkRenderTask {
            ...
            public SortTransparencyTask(ChunkPos p_i242101_2_, double p_i242101_3_,
                    ChunkRenderDispatcher.CompiledChunk p_i242101_5_) {
            ...
            public CompletableFuture<ChunkRenderDispatcher.ChunkTaskResult> execute(
                ...
                if (this.finished.get()) {
                    ...
                    if (bufferbuilder$state != null
                            && this.sortCompiledChunk.layersUsed.contains(RenderType.getTranslucent())) {
                        ...
                        bufferbuilder.sortVertexData(
                                (float) ChunkRender.this.regionDX + f - (float) ChunkRender.this.position.getX(),
                                (float) ChunkRender.this.regionDY + f1 - (float) ChunkRender.this.position.getY(),
                                (float) ChunkRender.this.regionDZ + f2 - (float) ChunkRender.this.position.getZ());
                        ...
                        if (this.finished.get()) {
                            ...
                            CompletableFuture<ChunkRenderDispatcher.ChunkTaskResult> completablefuture = ChunkRenderDispatcher.this
                                    .uploadChunkLayer(builderIn.getBuilder(RenderType.getTranslucent()),
                                            ChunkRender.this.getVertexBuffer(RenderType.getTranslucent()))
                                    .thenApply((p_lambda$execute$0_0_) -> {
                                        return ChunkRenderDispatcher.ChunkTaskResult.CANCELLED;
                                    });
                            return completablefuture.handle((p_lambda$execute$1_1_, p_lambda$execute$1_2_) -> {
                                if (p_lambda$execute$1_2_ != null
                                        && !(p_lambda$execute$1_2_ instanceof CancellationException)
                                        && !(p_lambda$execute$1_2_ instanceof InterruptedException)) {
                                    Minecraft.getInstance().crashed(
                                            CrashReport.makeCrashReport(p_lambda$execute$1_2_, "Rendering chunk"));
                                }

                                return this.finished.get() ? ChunkRenderDispatcher.ChunkTaskResult.CANCELLED
