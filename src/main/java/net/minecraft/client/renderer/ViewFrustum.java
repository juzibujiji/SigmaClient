...
public class ViewFrustum {
    ...
    public ViewFrustum(ChunkRenderDispatcher renderDispatcherIn, World worldIn, int countChunksIn,
    ...
    protected void createRenderChunks(ChunkRenderDispatcher renderChunkFactory) {
        ...
        for (int j = 0; j < this.countChunksX; ++j) {
            for (int k = 0; k < this.countChunksY; ++k) {
                for (int l = 0; l < this.countChunksZ; ++l) {
                    ...
                    this.renderChunks[i1].setPosition(j * 16, (k - WorldHeightHelper.getSectionOffset()) * 16, l * 16);
        ...
        for (int j1 = 0; j1 < this.renderChunks.length; ++j1) {
            ...
            for (int k1 = 0; k1 < Direction.VALUES.length; ++k1) {
                ...
                chunkrenderdispatcher$chunkrender1.setRenderChunkNeighbour(direction,
    ...
    protected void setCountChunksXYZ(int renderDistanceChunks) {
        ...
        this.countChunksX = i;
                this.countChunksY = WorldHeightHelper.getSectionCount();
        this.countChunksZ = i;
    }
        ...
        for (int k = 0; k < this.countChunksX; ++k) {
            ...
            for (int k1 = 0; k1 < this.countChunksZ; ++k1) {
                ...
                for (int k2 = 0; k2 < this.countChunksY; ++k2) {
                    int l2 = (k2 - WorldHeightHelper.getSectionOffset()) * 16;
                    ChunkRenderDispatcher.ChunkRender chunkrenderdispatcher$chunkrender = this.renderChunks[this
                            .getIndex(k, k2, k1)];
    ...
    public void markForRerender(int sectionX, int sectionY, int sectionZ, boolean rerenderOnMainThread) {
        ...
        int j = sectionY + WorldHeightHelper.getSectionOffset();
        int k = Math.floorMod(sectionZ, this.countChunksZ);
        if (j >= 0 && j < this.countChunksY) {
            ChunkRenderDispatcher.ChunkRender chunkrenderdispatcher$chunkrender = this.renderChunks[this.getIndex(i, j,
                    k)];
    ...
    public ChunkRenderDispatcher.ChunkRender getRenderChunk(BlockPos pos) {
        int i = pos.getX() >> 4;
        int j = (pos.getY() >> 4) + WorldHeightHelper.getSectionOffset();
        int k = pos.getZ() >> 4;
